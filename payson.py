...
class OrderItem(object):
    ...
    def __init__(self,
        ...
        optionals = [v for v in [sku, quantity, unitPrice,taxPercentage] if v]  #filter(lambda v: v is not None, [sku, quantity, unitPrice,
                                                     #taxPercentage])
        if len(optionals) != 0 and len(optionals) != 4:
            raise ValueError('Either all or none of sku, quantity, unitPrice '
                             'and taxPercentage must be provided')
        self.sku = sku
