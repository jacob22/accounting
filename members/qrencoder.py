...
class QRCode:
    ...
    def addData(self, data):
        if isinstance(data, QR):
            ...
            for conv in (QRNumber, QRAlphaNum, QRKanji, QR8bitByte):
                ...
                raise # Reraise the last ValueError
    ...
    def getBestMaskPattern(self):
        ...
        for i in range(8):
            ...
            transposedmodules = list(zip(*self.modules))
            lostPoint = QRUtil.getLostPoint(self.modules, transposedmodules,
                                            minLostPoint)
...
class QRUtil(object):
    ...
    @classmethod
    def getLostPoint(cls, modules, transposedmodules, minLostPoint=None):
        lostPoint = 0
        for fn, arg in ((cls.maskScoreRule4, modules),
                        (cls.maskScoreRule2, modules),
                        (cls.maskScoreRule3hor, modules),
                        (cls.maskScoreRule3hor, transposedmodules),
                        (cls.maskScoreRule1vert, modules),
                        (cls.maskScoreRule1vert, transposedmodules)):

            lostPoint += fn(arg)
            if minLostPoint is not None and lostPoint > minLostPoint:
                return lostPoint
...
EXP_TABLE = [ 1 << i for i in range(8) ] + [None] * (256-8)
